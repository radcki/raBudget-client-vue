...
export default {
    ...
    computed: {
        ...
        pickerType(){return this.step == 'months' ? 'month' : 'date'},
        addSign(){return this.step == 'months' ? 'M' : 'd'},
        minMoment(){return this.$moment(this.min, this.format)},
        maxMoment(){return this.$moment(this.max, this.format)},
        rangeDiff(){return this.maxMoment.diff(this.minMoment, this.step)},
        selectedMinMoment(){return this.$moment(this.selectedMin, this.format)},
        selectedMaxMoment(){return this.$moment(this.selectedMax, this.format)},
    ...
    watch: {
        ...
        period: function(value) {
            ...
            if (value == "6m") {
                this.selectedMin = this.$moment().subtract(6, "month").format("YYYY-MM");
                this.selectedMax = this.max;
            }
            if (value == "1m") {
                this.selectedMin = this.$moment().subtract(1, "month").format("YYYY-MM");
                this.selectedMax = this.max;
            }
        },
    ...
    methods:{
        intToDate(value){
            return this.minMoment.clone().add(value, this.addSign).format(this.format)
        },
        dateToInt(value){
        ...
        checkChips(range){
            ...
            if (range[0] == this.$moment().subtract(6, "month").format("YYYY-MM")){
                this.period = "6m";
                return
            }
            if (range[0] == this.$moment().subtract(1, "month").format("YYYY-MM")){
                this.period = "1m";
                return
            }      
            this.period = null;

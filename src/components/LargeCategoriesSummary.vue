    ...
    <v-layout row wrap justify-center>
      ...
      <template v-for="(category, index) in dataBalance">
        ...
        <v-flex :key="index + '_balance'" sm3 xs6>
          <div v-if="category.budgetSoFar != 0" class="text-xs-center caption">
            <v-animated-number
          ...
          <v-tooltip bottom>
            <template v-slot:activator="{}">
              <v-progress-linear
                ...
                :color="
                  conditionalColor((100 * category.overallBudgetBalance) / category.thisMonthBudget)
                "
              ></v-progress-linear>
            </template>
            <span>
              {{ (category.overallBudgetBalance / category.thisMonthBudget) | percentage }}
              <span v-if="category.thisYearYetScheduledSum > 0">
                ({{ $t('transactionSchedules.inclScheduled') }}:
                {{
                  ((category.overallBudgetBalance - category.thisMonthYetScheduledSum) /
                    category.thisMonthBudget)
                    | percentage
                }})
              </span>
            </span>
          ...
          <v-progress-linear
            ...
            :height="6"
            :value="
              (100 * (category.overallBudgetBalance - category.thisMonthYetScheduledSum)) /
                category.thisMonthBudget
            "
            :color="
              conditionalColor(
        ...
        <v-flex :key="index + '_month'" sm3 xs6>
          ...
          <v-tooltip bottom>
            <template v-slot:activator="{}">
              <v-progress-linear
                ...
                :color="
                  conditionalColor(
                    100 * (1 - category.thisMonthTransactionsSum / category.thisMonthBudget),
                  )
                "
              ></v-progress-linear>
            </template>
            <span>
              ...
              <span v-if="category.thisMonthYetScheduledSum > 0">
                ({{ $t('transactionSchedules.inclScheduled') }}:
                {{
          ...
          <v-progress-linear
            ...
            :height="6"
            :value="
              (100 * (category.thisMonthTransactionsSum + category.thisMonthYetScheduledSum)) /
                category.thisMonthBudget
            "
            :color="
              conditionalColor(
        ...
        <v-flex :key="index + '_year'" sm3 xs6>
          ...
          <v-tooltip bottom>
            <template v-slot:activator="{}">
              <v-progress-linear
                ...
                :color="
                  conditionalColor((100 * category.leftToEndOfYear) / category.thisYearBudget)
                "
              ></v-progress-linear>
            </template>
            <span>
          ...
          <v-progress-linear
            ...
            :height="6"
            :value="
              (100 * (category.leftToEndOfYear - category.thisYearYetScheduledSum)) /
                category.thisYearBudget
            "
            :color="
              conditionalColor(

<template>
  <v-container class="elevation-1 white">     
    <v-layout row wrap justify-center>
      ...
      <v-progress-circular v-if="!dataBalance" indeterminate color="primary"></v-progress-circular>
      ...
      <v-flex :key="index+'_balance'" sm3 xs6 >
        <div class="text-xs-center caption" v-if="category.budgetSoFar != 0">
          {{category.overallBudgetBalance | currency($currencies[dataBudget.currency])}}
        </div>
        <v-tooltip bottom>
        <v-progress-linear
        class="ma-0"
      ...
      <v-flex :key="index+'_month'"  sm3 xs6 >
        <div  class="text-xs-center caption">
          {{category.thisMonthTransactionsSum | currency($currencies[dataBudget.currency])}}
        </div>
        <v-tooltip bottom>
        <v-progress-linear
          class="ma-0"
      ...
      <v-flex :key="index+'_year'"  sm3 xs6 >
        <div v-if="category.thisYearBudget != 0" class="text-xs-center caption">
          {{category.leftToEndOfYear | currency($currencies[dataBudget.currency])}}
          </div>
        <v-tooltip bottom>
        <v-progress-linear
          class="ma-0"

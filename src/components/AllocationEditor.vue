...
export default class TransactionEditor extends Vue {
  ...
  mounted() {
    ...
    this.editor.targetBudgetCategory = this.categories.find(
      v => v.budgetCategoryId == this.value.targetBudgetCategoryId,
    );
    if (this.value.sourceBudgetCategoryId) {
      this.editor.sourceBudgetCategory = this.categories.find(
        v => v.budgetCategoryId == this.value.sourceBudgetCategoryId,
      );
    }
  }

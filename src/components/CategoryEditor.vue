<template>
  <v-dialog width="600" v-model="dialog" :fullscreen="mobile">
          ...
          <v-container grid-list-md>
            <v-layout row wrap>
              ...
              <v-flex xs12 class="text-xs-right">
                <v-subheader class="text-xs-right"><v-spacer></v-spacer><v-icon class="mr-3">payment</v-icon>{{$t('categories.monthlyAmount')}}</v-subheader>
                <v-btn @click="addPeriod" small color="primary" class="mb-4">+ {{$t("categories.addPeriod")}}</v-btn>
                <template v-for="(amountConfig, amountConfigIndex) in category.amountConfigs">
                  ...
                  <v-container :key="'row'+amountConfigIndex" grid-list-md class="pa-0 ma-0">
                    <v-layout row wrap class="pa-0 ma-0">                        
                      <v-flex :xs4="!mobile" :xs9="mobile">
                        ...
                        >
                          <v-text-field
                            ...
                            readonly
                          ></v-text-field>
                          <v-date-picker v-model="amountConfig.validFrom" :min="dataBudget.startingDate" type="month" @input="$refs.dateMenu[amountConfigIndex].save(amountConfig.validFrom)"></v-date-picker>
                        </v-menu>
                      </v-flex>

                      <v-flex :xs4="!mobile" :xs9="mobile">
                        <v-text-field :rules="requiredRule" v-model="amountConfig.amount" :label="$t('categories.monthlyAmount')"></v-text-field>
                      </v-flex>
                      ...
                      <v-flex xs1>
                        <v-btn icon color="red" v-if="category.amountConfigs.length > 1" small dark @click="category.amountConfigs.splice(amountConfigIndex, 1)"><v-icon >close</v-icon></v-btn>
                      </v-flex>

                      <v-flex xs9 v-if="mobile" class="pb-3">
                        <div>
                          {{$t("categories.daily")}}: {{ (amountConfig.amount / 30) | currency($currencies[dataBudget.currency]) }}</div><div>
                          {{$t("categories.annual")}}: {{ (amountConfig.amount * 12) | currency($currencies[dataBudget.currency]) }}
                        </div>
                      </v-flex>
...
export default {
  ...
  watch:{
    dialog(open){
      ...
      setTimeout(()=>{
        this.category = { ...this.category, ...JSON.parse(JSON.stringify(this.value))}
      }, 500)      
    }
  },
  ...
  mounted: function() {
    ...
    for (var config of this.category.amountConfigs) {
      ...
      config.validTo = config.validTo ? this.$moment(config.validTo).format("YYYY-MM") : null;
    }
  },
  computed: {
    mobile(){return !this.$vuetify.breakpoint.smAndUp}
  },
  methods: {
    addPeriod() {
